...
import { Course, CourseDate, CourseTimeSlot } from "@prisma/client";
import { H2, H3 } from "@/components/ui/Heading";
import { savePendingEnrollment, recordPayment } from "./actions";
...
type ExtendedCourse = Course & { dates: (CourseDate & { timeSlots: CourseTimeSlot[] })[] };

export default function RegisterClient({ course }: { course: ExtendedCourse }) {
  ...
  async function handleDetailsSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await savePendingEnrollment(formData, course.id);
