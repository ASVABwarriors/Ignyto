...
export default function GradeWiseCampsPaymentsTableClient({ initialPayments }: { initialPayments: PaymentWithRelations[] }) {
  ...
  const filteredPayments = initialPayments.filter((p) => {
    ...
    const courseTitle = p.gradeWiseCamp?.title?.toLowerCase() || "";
  ...
  const downloadCSV = () => {
    ...
    const rows = filteredPayments.map((p, index) => [
      ...
      escapeCSV(p.gradeWiseCamp?.title || ""),
          ...
          <tbody>
            ...
            {filteredPayments.map((p, index) => (
              <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50/50 align-top">
                ...
                <td className="py-4 px-6">
                  <H4 className="font-bold text-primary-dark">{p.gradeWiseCamp?.title}</H4>
                  {p.enrollment && (
                    <div className="mt-1">
